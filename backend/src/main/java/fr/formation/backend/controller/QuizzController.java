...
public class QuizzController {
  ...
  @Autowired private IstRepository istRepository;
  ...
  public List<IstResponse> findThreeRandom(@AuthenticationPrincipal CustomUserDetails userDetails) {
    ...
    List<Ist> istRandom =
        this.istRepository.findThreeRandom(); // .stream().map(IstResponse::convert).toList();
    ...
    List<Ist> userIstList = user.getIsts();
    userIstList.addAll(istRandom);
    user.setIsts(userIstList);
  ...
  public ResponseEntity<Object> increaseLevel(
  ...
  public ResponseEntity<Object> postMethodName(
      @PathVariable @NonNull Integer idCard,
      @AuthenticationPrincipal CustomUserDetails userDetails) {
    ...
    List<Ist> istList = user.getIsts();
    istList.add(ist);
    user.setIsts(istList);
